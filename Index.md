**CharacterSheet.md** — Gerald's stats, meters, assets, vow, connections, legacy tracks
**Background_Character.md** — Gerald's full backstory
**Background_History.md** — History of the Divide
**Background_Current.md** — Current state of the Divide
**Background_Geography.md** — Geography of the Divide
**Background_ShipTech.md** — Ship technology and capabilities
**Background_Weaponry.md** — Weapon systems
**Background_Notables.md** — Notable Historical Figures
**InterestingObservations.md** — Staged setting inferences, tagged Certain or Speculative
**Ship_Details.md** — The Steady Return: capabilities, armament, crew
**Rules.md** — Core mechanics: rolls, progress tracks, momentum, impacts
**Moves.md** — Full move list
**Moves_Glossary.md** — Mechanical term definitions
**GameState.md** — Current tactical situation, key personnel, narrator background knowledge
**Meta_LongTermPlanning.md** — Vow milestone outline, long-term campaign threads
**CheckQuestions.md** — Verification questions for session readiness
**Prompt.txt** — GM instructions, mode toggle, roll protocol, beat markers
**Directives.txt** — Hard constraints on narrative
**Nextsession.txt** — Session 5 briefing
**SessionLog_1.md** — Arrival, the vow, the standoff, departure prep
**SessionLog_2.md** — Descent, surface approach, entry, meeting Carl
**SessionLog_3.md** — Carl's intelligence, Hamzi's arrival and the data pad
**SessionLog_4.md** — Sandra Vael recruited, precursor formation revealed
**SessionLog_5.md** — Secure comms established, Aran Chrion introduced